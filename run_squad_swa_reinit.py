...
#     authorized_unexpected_keys = [r"pooler"]
    ...
    def __init__(self, config, reinit_n_layers=1):
...
def main():
    ...
    model = ReinitWeightQA.from_pretrained(
...
#     print(model.bert.encoder.layer[5].attention.output.dense.weight.data)
    ...
    if args.do_train:
        ...
        global_step, tr_loss, swa_model = train(args, train_dataset, model, tokenizer)
...
#     print("Results: {}".format(results))
    ...
    ## SWA
    results = {}
    result = evaluate(args, swa_model, tokenizer, prefix=global_step)
    result = dict((k + ("_{}".format(global_step) if global_step else ""), v) for k, v in result.items())
    results.update(result)
    print("SWA Results: {}".format(results))
