...
def main():
    ...
    if data_args.task_name is not None:
        ...
        print('datasets.keys =', datasets['train'].features.keys())
    else:
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        preds = np.squeeze(probs) if is_regression else np.argmax(probs, axis=1)
        if data_args.task_name is not None:
            ...
            return {"acc": simple_accuracy(preds, p.label_ids, probs)} # hiepnh
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            # hiepnh
            pickle.dump(predictions, open('result.pickle', 'wb'))
