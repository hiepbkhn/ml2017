...
tf.app.flags.DEFINE_string('bert_base_dir', '../BERT/chinese_L-12_H-768_A-12/', 'base dir of pretrained bert')
...
def build_model(word_embedding, x_bert, x_mask_bert, x_type_bert, s_idx_bert, x, sen_len, doc_len, is_training):
    ...
    tf.logging.info('building subtasks')
    ...
    tf.logging.info('build subtasks Done!')
...
def run():
    ...
    # Model Code Block
    word_idx_rev, word_id_mapping, word_embedding, _ = load_w2v(embedding_dim=200, embedding_dim_pos=50, data_file_path='data/all_data_pair.txt', embedding_path='data/w2v_200.txt')
    ...
    tf.logging.info('build model done!\n')
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        while True:
            ...
            if fold == FLAGS.end_fold: 
                break
            tf.logging.info('############# fold {} begin ###############'.format(fold))
            init_from_bert_checkpoint()
            sess.run(tf.global_variables_initializer())

            # Data Code Block
            train_file_name = 'data/{}/fold{}_train.txt'.format(FLAGS.split, fold)
            dev_file_name = 'data/{}/fold{}_valid.txt'.format(FLAGS.split, fold)
            test_file_name = 'data/{}/fold{}_test.txt'.format(FLAGS.split, fold)
            ...
            if FLAGS.split=='split20':
                ...
                dev_data = test_data
            tf.logging.info('train docs: {}  valid docs: {}  test docs: {}'.format(len(train_data.x), len(dev_data.x), len(test_data.x)))
            ...
            for i in range(FLAGS.training_iter):
                ...
                def combine_eval_inputs(x):
                    ...
                    for a in list(x):
                        ret.extend(list(a))
                    return np.array(ret) 
                def print_result(emo_cause_prf, pair_prf):
                    p, r, f1 = emo_cause_prf[:3]
                    tf.logging.info('emotion_prediction: p {:.4f} r {:.4f} f1 {:.4f}'.format(p, r, f1 ))
                    p, r, f1 = emo_cause_prf[3:6]
                    tf.logging.info('cause_prediction: p {:.4f} r {:.4f} f1 {:.4f}'.format(p, r, f1 ))
                    p, r, f1 = pair_prf[:3]
                    tf.logging.info('row pair_prediction: p {:.4f} r {:.4f} f1 {:.4f}'.format(p, r, f1 ))
                    p, r, f1 = pair_prf[3:6]
                    tf.logging.info('col pair_prediction: p {:.4f} r {:.4f} f1 {:.4f}'.format(p, r, f1 ))
                ...
                def evaluation(dataset):
                    ...
                    batch_eval_inputs = zip(*batch_eval_inputs)
                    eval_input = map(combine_eval_inputs, batch_eval_inputs)
                    emo_cause_prf, pair_prf = cal_ecp_prf(eval_input, ds.pair_left_cnt, threshold = FLAGS.threshold, window_size =FLAGS.window_size)
                    return emo_cause_prf, pair_prf

                de_emo_cause_prf, de_pair_prf = evaluation(dev_data)
                te_emo_cause_prf, te_pair_prf = evaluation(test_data)
                ...
                tf.logging.info('Dev emotion_prediction: p {:.4f} r {:.4f} f1 {:.4f}'.format(p, r, f1 ))
                p, r, f1 = de_emo_cause_max_prf[:3]
                tf.logging.info('Dev emotion_prediction: max_p {:.4f} max_r {:.4f} max_f1 {:.4f}'.format(p, r, f1))
                p, r, f1 = te_emo_cause_max_prf[:3]
                tf.logging.info('Test emotion_prediction: max_p {:.4f} max_r {:.4f} max_f1 {:.4f}\n'.format(p, r, f1))

                p, r, f1 = de_emo_cause_prf[3:6]
                tf.logging.info('Dev cause_prediction: p {:.4f} r {:.4f} f1 {:.4f}'.format(p, r, f1 ))
                p, r, f1 = de_emo_cause_max_prf[3:6]
                tf.logging.info('Dev cause_prediction: max_p {:.4f} max_r {:.4f} max_f1 {:.4f}'.format(p, r, f1))
                p, r, f1 = te_emo_cause_max_prf[3:6]
                tf.logging.info('Test cause_prediction: max_p {:.4f} max_r {:.4f} max_f1 {:.4f}\n'.format(p, r, f1))
            ...
            tf.logging.info('############# fold {} end ###############'.format(fold))
        ...
        def finnal_output(emo_cause_list, pair_list):
            for i in range(5):
                ...
                tf.logging.info('average : p {:.4f} r {:.4f} f1 {:.4f}\n'.format(p, r, f1))
        ...
        tf.logging.info('############# Evaluation on Dev ############# ')
        finnal_output(de_emo_cause_list, de_pair_list)
        tf.logging.info('############# Evaluation on Test ############# ')
...
def main(_):
    ...
    ########################## Split 10 ##########################
    FLAGS.split='split10'
    FLAGS.start_fold, FLAGS.end_fold = 1, 11
    ...
    FLAGS.training_iter, FLAGS.batch_size, FLAGS.learning_rate = 20, 8, 2e-5
