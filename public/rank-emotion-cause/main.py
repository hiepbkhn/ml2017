    ...
    for epoch in range(1, configs.epochs+1):
        ...
        with torch.no_grad():
            ...
            if configs.split == 'split10':
                # hiepnh
                configs.fold_id = fold_id
                configs.epoch_id = epoch

                test_ec, test_e, test_c, _, _, _ = inference_one_epoch(configs, test_loader, model)
                # hiepnh
                print('test_ec : %.3f %.3f %.3f'%(test_ec[0], test_ec[1], test_ec[2]))
                print('test_e  : %.3f %.3f %.3f'%(test_e[0], test_e[1], test_e[2]))
                print('test_c  : %.3f %.3f %.3f'%(test_c[0], test_c[1], test_c[2]))
...
def inference_one_batch(configs, batch, model):
    ...
    return to_np(loss_couple), to_np(loss_e), to_np(loss_c), \
           doc_couples_b, doc_couples_pred_b, doc_id_b, pred_e, pred_c
...
def inference_one_epoch(configs, batches, model):
    ...
    for batch in batches:
        _, _, _, doc_couples, doc_couples_pred, doc_id_b, pred_e, pred_c = inference_one_batch(configs, batch, model)
        doc_id_all.extend(doc_id_b)
        doc_couples_all.extend(doc_couples)
        doc_couples_pred_all.extend(doc_couples_pred)
        pred_e_all.append(pred_e.cpu().numpy())
        pred_c_all.append(pred_c.cpu().numpy())
    # hiepnh
    pickle.dump((pred_e_all, pred_c_all, doc_id_all), open(str(configs.fold_id) + '-' + str(configs.epoch_id) + '.pkl', 'wb'))
