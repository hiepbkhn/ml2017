...
class Encoder(nn.Module):
    def __init__(self, chs=(768,64,128)):
        super().__init__()
...
class BertUNetQA(BertPreTrainedModel):

    authorized_unexpected_keys = [r"pooler"]

    def __init__(self, config, enc_chs=(768,64,128), dec_chs=(128, 64)):
        super().__init__(config)
        self.num_labels = config.num_labels
        # UNet
        self.encoder     = Encoder(enc_chs)
        self.decoder     = Decoder(dec_chs)
        self.head        = nn.Conv1d(dec_chs[-1], self.num_labels, 1) # kernel-size = 1
        # BERT
...
def main():
    ...
    model = BertUNetQA.from_pretrained(
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = BertUNetQA.from_pretrained(args.output_dir)  # , force_download=True)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = BertUNetQA.from_pretrained(checkpoint)  # , force_download=True)
            model.to(args.device)
