...
class CenterQA(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.new_outputs = nn.Linear(config.hidden_size, self.num_labels) # qa_outputs
...
def main():
    ...
    model = CenterQA.from_pretrained(
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = CenterQA.from_pretrained(args.output_dir)  # , force_download=True)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = CenterQA.from_pretrained(checkpoint)  # , force_download=True)
            model.to(args.device)
